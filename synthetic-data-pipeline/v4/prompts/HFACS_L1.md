...
An omission qualifies as an active failure only if the omission itself is the mechanism that produced the fatal outcome. Failing to deploy the parachute qualifies because not deploying is what killed the jumper.
...
- **Visual illusion:** Environmental conditions (flat light, snow cover, leafless vegetation, featureless terrain, fog blending sky and ground) caused the jumper to misperceive altitude, distance, or terrain proximity.
- **Vestibular/spatial disorientation:** Loss of spatial orientation during tumble, spin, or in visually impoverished conditions caused the jumper to misperceive their body position, heading, or proximity to the object.
