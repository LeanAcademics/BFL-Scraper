#!/usr/bin/env python3
"""
Compare this run's classification results against the shared ground truth.

Reads:  ./output/classification_results.jsonl
        ../synthetic-data-output/ground-truth.jsonl
...
GT_FILE = PIPELINE_DIR / "synthetic-data-output" / "ground-truth.jsonl"
...
def _parent_category(code):
    """Derive L2 parent category from a (possibly sub-)category code.

    Model outputs sometimes fill `category` with a subcategory code
    (e.g. "201C2" instead of "201C"). Normalize to the parent prefix:
    201C* -> 201C, 202P* -> 202P, 203T -> 203T, 204E -> 204E.
    """
    if not code:
        return code
...
def main():
    ...
    # -----------------------------------------------------------------------
    print(f"\n{'='*70}")
    print(f"  Comparison: claude-new-prompts-no-thinking-temp0.3 vs ground truth "
          f"({len(comparison)} records)")
    print(f"{'='*70}\n")
