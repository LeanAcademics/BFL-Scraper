#!/usr/bin/env python3
"""
HFACS-BASE Synthetic Validation Pipeline — new2-prompts / no-thinking / temp=0.3.

Isolated run under new2-prompts-no-thinking-temp0.3/. Reads prompts from the
LOCAL ./prompts/ folder (split from HFACS-BASE-NEW2.md), domain context from
the main classification-pipeline, and the shared synthetic-bfl input. Writes
all outputs into ./output/ within this subfolder.

Differences from the main synthetic script:
- Prompts come from LOCAL ./prompts/, not classification-pipeline/prompts/.
- Extended thinking is disabled.
- Sampling temperature is set to 0.3.
"""
...
MAIN_PIPELINE_DIR = REPO_ROOT / "classification-pipeline"
...
def parse_response(response):
    """
    Extract classification JSON from an API response.
    ...
    # Find the first '{' and use raw_decode so trailing content (extra prose
    # or a second object after the JSON) doesn't cause a parse failure.
...
def call_live(client, system_prompt, user_msg):
    """
    Make a single API call using streaming.

    Streaming is required by the SDK because the combined ~28k-token system
    prompt + max_tokens=35000 may exceed the 10-minute non-streaming limit.
    Do not switch to messages.create — it will raise
...
def run_layer(client, layer, records, l0_results=None, l1_results=None, only=None):
    """
    Run classification for one layer.
    Returns dict of {record_id: parsed_result}.

    If `only` is a set of record IDs, only those records are processed this run.
    Existing results in L{layer}_results.jsonl are preserved and the processed
    records are merged in (replacing any prior entry).
    """
    if layer == "L0":
        eligible = records
    elif layer == "L1":
    ...
    # Merge with existing results on disk so a targeted --only re-run doesn't
    # wipe the other records. Newly-processed records replace their prior entry.
...
def merge_results():
    """Merge L0/L1/L2 results into final classification_results.jsonl."""
...
def main():
    parser = argparse.ArgumentParser(
        description="HFACS-BASE synthetic validation — no-thinking / temp=0.3"
    )
    ...
    parser.add_argument("--only", type=str, default=None,
                        help="Comma-separated record IDs to re-run (e.g. SYN-CS12,SYN-CS17). "
                             "Existing results for other records are preserved; new results "
                             "for --only records replace their prior entry.")
    ...
    log.info("Config: model=%s, thinking=OFF, temperature=%s", MODEL, TEMPERATURE)

    # Wipe errors.jsonl only on a full run. On --only, keep the prior log;
    # we'll prune entries for re-run records so their old failures don't linger.
    ...
    for layer in layers_to_run:
        # On --only targeted re-runs, we typically run a single later layer and
        # need prior-layer results from disk.
        ...
        # Refresh in-memory view from disk so the next layer sees merged state.
        if layer == "L0":
