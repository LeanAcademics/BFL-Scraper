#!/usr/bin/env python3
"""
HFACS-BASE Synthetic Validation Pipeline — v5-new-HFACS.

Reads all prompts, the domain reference, the label-example reference, and the
synthetic BFL input from the LOCAL v5-new-HFACS/ folder. Writes outputs into
./output/ within this folder.

Differences vs. v4:
- Prompt files live at the folder root (no ./prompts/ subdir).
- New reference file CATEGORIES_EXAMPLES_CONTEXT.md injected between the
  domain reference and the layer prompt.
- L2 eligibility excludes records whose only L1 act is 104II (Insufficient
  Information) — there is no identified unsafe act to be a precondition for.
"""
...
EXAMPLES_FILE = RUN_DIR / "CATEGORIES_EXAMPLES_CONTEXT.md"
HFACS_FILES = {
...
def build_system_prompt(layer):
    """
    Assemble system prompt in the v5 order:
      base_system -> BASE_DOMAIN_CONTEXT -> CATEGORIES_EXAMPLES_CONTEXT -> HFACS_L{layer}

    Examples come BEFORE the layer prompt so the output schema stays last,
    and the label catalog reads as reference rather than authoritative anchor.
    One ephemeral cache_control block with 1h TTL — the whole prefix is read
    from cache on every record after the first.
    """
...
def call_live(client, system_prompt, user_msg):
    ...
    Streaming is required: combined prefix + max_tokens may exceed the
    10-minute non-streaming limit. Do not switch to messages.create.
    """
...
def _l1_has_actionable_act(l1_rec):
    """True iff L1 result contains at least one non-104II unsafe act."""
    acts = l1_rec.get("L1_unsafe_acts") or []
    return any(a.get("category") != "104II" for a in acts)
...
def run_layer(client, layer, records, l0_results=None, l1_results=None, only=None):
    ...
    if layer == "L0":
        ...
        log.info("L2: %d records with at least one non-104II L1 act", len(eligible))
...
def main():
    parser = argparse.ArgumentParser(
        description="HFACS-BASE synthetic validation — v5-new-HFACS"
    )
