#!/usr/bin/env python3
"""
Compare this v5-new-HFACS run's classification results against the local
ground truth.
...
def extract_l2_subcategories(record):
    ...
    if preconds is None:
        return None
    # Subcategory is null for 203T and 204E in the new schema; fall back
    # to the parent category so those still appear in the subcategory view.
    subs = set()
    for p in preconds:
...
def _parent_category(code):
    """Normalize to the parent L2 category prefix."""
    if not code:
        return code
...
def main():
    ...
    # -----------------------------------------------------------------------
    print(f"\n{'='*70}")
    print(f"  Comparison: v5-new-HFACS vs ground truth "
    ...
    # Per-class L1 breakdown (including 104II)
    l1_classes = ["101D", "102S", "104II"]
    print()
